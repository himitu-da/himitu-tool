...
export default function Hex() {
  ...
  const run = () => { try { setOut(inp.split("").map(c=>c.charCodeAt(0).toString(16).padStart(2,"0")).join(" ")) } catch(e) { setOut("エラー"); } };
  return (
    <>
      <ToolStickyHeader title="テキスト → 16進数" className="bg-gray-800 text-white" />
