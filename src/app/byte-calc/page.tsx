...
export default function ByteCalcPage() {
  ...
  const UnitDropdownOptions = () => (
    <>
      <optgroup label="SI接頭語 (1000倍) - バイト">
        {MAGS.map(m => <option key={`si:byte:${m}`} value={`si:byte:${m}`}>{getReading("si", "byte", m)} ({getSymbol("si", "byte", m)})</option>)}
      </optgroup>
      <optgroup label="2進接頭辞 (1024倍) - バイト">
        {MAGS.map(m => <option key={`binary:byte:${m}`} value={`binary:byte:${m}`}>{getReading("binary", "byte", m)} ({getSymbol("binary", "byte", m)})</option>)}
      </optgroup>
      <optgroup label="SI接頭語 (1000倍) - ビット">
        {MAGS.map(m => <option key={`si:bit:${m}`} value={`si:bit:${m}`}>{getReading("si", "bit", m)} ({getSymbol("si", "bit", m)})</option>)}
      </optgroup>
      <optgroup label="2進接頭辞 (1024倍) - ビット">
        {MAGS.map(m => <option key={`binary:bit:${m}`} value={`binary:bit:${m}`}>{getReading("binary", "bit", m)} ({getSymbol("binary", "bit", m)})</option>)}
      </optgroup>
    </>
  );
  ...
  const DetailedUnitSelector = ({ idPrefix, prefix, setPrefix, type, setType, mag, setMag }: DetailedUnitSelectorProps) => (
    <div className={`p-4 rounded-xl space-y-4 ${blockCls} shadow-inner`}>
      <div className="font-bold border-b border-black/5 dark:border-white/10 pb-2">
        選択肢より指定
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <ToolRadio name={`${idPrefix}_p`} variant="card" checked={prefix === "si"} onChange={() => setPrefix("si")}
            label={<div className="text-center space-y-0.5"><div className="font-bold">SI接頭語</div><div className="text-xs opacity-70 font-mono tracking-tighter">10³<br/>1000倍</div></div>} />
          <ToolRadio name={`${idPrefix}_p`} variant="card" checked={prefix === "binary"} onChange={() => setPrefix("binary")}
            label={<div className="text-center space-y-0.5"><div className="font-bold">2進接頭辞</div><div className="text-xs opacity-70 font-mono tracking-tighter">2¹⁰<br/>1024倍</div></div>} />
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <ToolRadio name={`${idPrefix}_t`} variant="radiocard" checked={type === "byte"} onChange={() => setType("byte")} label="バイト" />
          <ToolRadio name={`${idPrefix}_t`} variant="radiocard" checked={type === "bit"} onChange={() => setType("bit")} label="ビット" />
        </div>
      </div>
  ...
  return (
    <ToolPageLayout title="バイト単位変換" maxWidth="5xl">
      ...
      <div className={`mb-6 p-4 md:p-6 rounded-2xl ${blockCls} shadow-inner flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 border border-black/5 dark:border-white/5`}>
        ...
        <ToolColumn>
          <ToolPanel title="単位 1">
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="block text-sm font-semibold">直接指定</label>
                <div className="flex gap-2">
                  ...
                  <select
                    ...
                    className={`h-12 px-2 py-0 rounded-xl border focus:ring-2 outline-none cursor-pointer w-[140px] shrink-0 font-bold ${inputCls}`}
                  >
                    <UnitDropdownOptions />
                  </select>
                </div>
                <div className="flex gap-2">
                  <ToolButton variant="secondary" onClick={() => handlePaste("1")} className="px-4 py-2 text-sm flex-1" title="ペースト">
                    <ClipboardPaste size={16} /> <span className="hidden sm:inline">ペースト</span>
                  </ToolButton>
                  <ToolButton variant="secondary" onClick={() => handleCopy("1")} className="px-4 py-2 text-sm flex-1" title="結果をコピー">
                    {copied1 ? <Check size={16} /> : <Copy size={16} />} <span className="hidden sm:inline">コピー</span>
                  </ToolButton>
                </div>
          ...
          <ToolPanel title="単位 2">
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <ToolButton variant="secondary" onClick={() => handlePaste("2")} className="px-4 py-2 text-sm flex-1" title="ペースト">
                    <ClipboardPaste size={16} /> <span className="hidden sm:inline">ペースト</span>
                  </ToolButton>
                  <ToolButton variant="secondary" onClick={() => handleCopy("2")} className="px-4 py-2 text-sm flex-1" title="結果をコピー">
                    {copied2 ? <Check size={16} /> : <Copy size={16} />} <span className="hidden sm:inline">コピー</span>
                  </ToolButton>
                </div>
