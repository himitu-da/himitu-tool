...
export default function HtmlE() {
  const [inp, setInp] = useState("");
  const [out, setOut] = useState("");
  const run = () => { try { setOut(inp.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/\'/g,"&#039;")) } catch(e) { setOut("エラー"); } };
  return (
    <>
      <ToolStickyHeader title="HTMLエンティティ化" className="bg-gray-800 text-white" />
      <div className="max-w-2xl mx-auto p-6 rounded-xl shadow-lg border border-opacity-20 border-current bg-white/10 backdrop-blur-sm mt-4">
        <div className="flex flex-col gap-4">
          ...
          <button onClick={run} className="py-3 bg-blue-500/80 hover:bg-blue-600/80 text-white rounded-lg font-bold transition-colors">置換</button>
