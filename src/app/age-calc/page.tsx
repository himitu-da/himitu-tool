...
export default function Age() {
  ...
  const run = () => { try { setOut((new Date().getFullYear()-Number(year))+"歳") } catch(e) { setOut("エラー"); } };
  return (
    <>
      <ToolStickyHeader title="年齢計算" className="bg-gray-800 text-white" />
      ...
      <div className="flex flex-col gap-4">
        <input type="number" value={year} onChange={e=>set_year(e.target.value)} placeholder="year" className="p-3 bg-black/10 rounded-lg text-current border-current" />
