...
export default function Ded() {
  ...
  const run = () => { try { setOut(Array.from(new Set(inp.split("\n"))).join("\n")) } catch(e) { setOut("エラー"); } };
  return (
    <>
      <ToolStickyHeader title="重複行の削除" className="bg-gray-800 text-white" />
      <div className="max-w-2xl mx-auto p-6 rounded-xl shadow-lg border border-opacity-20 border-current bg-white/10 backdrop-blur-sm mt-4">
        <div className="flex flex-col gap-4">
          ...
          <button onClick={run} className="py-3 bg-blue-500/80 hover:bg-blue-600/80 text-white rounded-lg font-bold transition-colors">削除</button>
