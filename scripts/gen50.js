...
const db = [
  ['b64-enc', 'Base64エンコード', '🔤', '文字をBase64に', TPL_TEXT('Base64エンコード', 'B64E', '変換', 'setOut(btoa(encodeURIComponent(inp).replace(/%([0-9A-F]{2})/g, (m, p1) => String.fromCharCode(parseInt(p1, 16)))))')],
  ...
  ['aspect-ratio', 'アスペクト比', '🖥️', 'W/Hの比率', TPL_NUM('アスペクト比', 'Ratio', ['w','h'], 'const gcd=(a: number,b: number): number=>b?gcd(b,a%b):a; const g=gcd(Number(w),Number(h)); setOut((Number(w)/g)+":"+(Number(h)/g))')],
  ['random-num', '乱数生成', '🎲', '範囲内の乱数', TPL_NUM('乱数生成', 'Rand', ['min','max'], 'setOut((Math.floor(Math.random()*(Number(max)-Number(min)+1))+Number(min)).toString())')],
